...
grade_update() {
    ...
    # Usage: grade_update test_name points_to_add possible_points expected_retcode
    return_status=$?
    check_hashes
    echo -e "\nTest for: $1"
    if [ "$return_status" == "$4" ]; then
        if [ "$2" -lt $3 ]; then
    ...
    ((perfect_points = perfect_points + $3))
}
...
unit_tests() {
    ...
    for testpath in $glob_expr; do
        ...
        total_points=$(grep ^"$filename" "unit_tests/weights.txt" | awk '{print $2'})
        if [ "$language" = "cpp" ]; then
        ...
        if [ $? -eq "$expected_retcode" ]; then
            grade_update "$filename" $total_points $total_points 0  # Passed, award full points
        else
            grade_update "$filename" 0 $total_points 0  # Failed, award 0
...
if [ "$enable_static_analysis" = true ]; then
    ...
    grade_update "static analysis / typechecking" 100 100 0
fi
######## <- Static analysis ########
...
if [ "$enable_format_check" = true ]; then
    ...
    grade_update "auto-format style check" 100 100 0
    shopt -u nullglob
fi
######## <- Format check ########
...
grade=$(echo "print(int(($grade / $perfect_points)*100))" | python3)
